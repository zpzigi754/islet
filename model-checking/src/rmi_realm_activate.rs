...
use crate::common::{pre_granule_state, pre_rd_state};
...
fn verify_realm_activate() {
    ...
    if prop_success_realm_state_ante {
        let success_realm_state_addr = get_granule!(rd).map(|guard| guard.index_to_addr()).unwrap();
        let success_realm_state_rd = unsafe { &*(success_realm_state_addr as *const Rd) };
        let success_realm_state_post = success_realm_state_rd.state() == State::Active;
